...
if [ $# -eq 0 ]; then
  ...
  echo -e " \n As for bashLED blink command \n  where command is one of the number  \n e.g. bashLED blink 5"
  exit 2
fi
...
if [ "$1" == "on" ]; then
  ...
  removeTrigger
  counter=$2
  while [ $counter -gt 0 ]			# Create a loop for number of times blinking
  do
      echo "1" >> "$LED3_PATH/brightness"	# Turn LED on
      sleep 1					# Wait for 1 second
      echo "0" >> "$LED3_PATH/brightness"	# Turn LED off
      sleep 1					# Wait for 1 second
      ((counter--))				# decrease number of times for blinking
done
elif [ "$1" == "status" ]; then
